...
describe('paymentRequestBalance actions', () => {
	describe('action creators', () => {
		it('should create paymentRequestBalance action objects', () => {
			...
			expect(paymentRequestBalanceActions.listPaymentRequestBalanceSucceeded(successPayload)).to.eql({
	...
	describe('async actions', () => {
		...
		it('creates LIST_PAYMENT_REQUEST_BALANCE_SUCCEEDED when listing balances succeeds', () => {
			...
			return store.dispatch<any>(paymentRequestBalanceActions.listPaymentRequestBalances()).then(() => {
		...
		it('creates LIST_PAYMENT_REQUEST_BALANCE_FAILED when listing balances fails (HTTP 500)', () => {
			...
			return store.dispatch<any>(paymentRequestBalanceActions.listPaymentRequestBalances()).then(() => {
				...
				expect(actions[0]).to.eql({ type: types.LIST_PAYMENT_REQUEST_BALANCE_REQUESTED, payload: undefined })
