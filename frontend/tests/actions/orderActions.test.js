...
describe('order actions', () => {
	describe('async actions', () => {
		...
		it('creates CREATE_ORDER_SUCCESS when creating order has been done', () => {
			...
			const expectedActions = [
				...
				{ type: typesNotification.ADD_NOTIFICATION, text: 'actions.order.create.success', open: true },
		...
		it('creates CREATE_ORDER_ERROR when creating order fails', () => {
			...
			const expectedActions = [
				{ type: types.CREATE_ORDER_REQUESTED, completed: false },
				{ type: typesNotification.ADD_NOTIFICATION, text: 'actions.order.create.payment.error', open: true },
				{ type: types.CREATE_ORDER_ERROR, completed: true, error: new Error('Request failed with status code 500') }
			]
			const store = mockStore({ intl: { messages: {} } })
		...
		it('creates LIST_ORDERS_ERROR when listing orders fails', () => {
			...
			const expectedActions = [
				...
				{ type: types.LIST_ORDERS_ERROR, completed: true, error: new Error('Request failed with status code 500') }
			]
			const store = mockStore({ intl: { messages: {} } })
			return store.dispatch(orderActions.listOrders()).then(() => {
				const actions = store.getActions()
    ...
    xit('creates DETAIL_ORDER_ERROR when detailing order fails', () => {
      ...
      const expectedActions = [
        { type: types.DETAILS_ORDER_REQUESTED, completed: false },
        { type: typesNotification.ADD_NOTIFICATION, text: 'actions.order.details.error', open: true },
        { type: types.DETAILS_ORDER_ERROR, completed: true, error: new Error('Request failed with status code 500') }
      ]
      const store = mockStore({ intl: { messages: {} } })
      return store.dispatch(orderActions.detailOrder(1)).then(() => {
        const actions = store.getActions()
    ...
    it('creates REFUND_ORDER_SUCCESS when refunding order has been done', () => {
      ...
      const expectedActions = [
        ...
        { type: typesNotification.ADD_NOTIFICATION, text: 'actions.order.refund.success', open: true },
    ...
    xit('creates REFUND_ORDER_ERROR when refunding order fails', () => {
      ...
      const expectedActions = [
        { type: types.REFUND_ORDER_REQUESTED, completed: false },
        { type: typesNotification.ADD_NOTIFICATION, text: 'actions.order.cancel.payment.error', open: true },
        { type: types.REFUND_ORDER_ERROR, completed: true, error: new Error('Request failed with status code 500') }
      ]
      const store = mockStore({ intl: { messages: {} } })
      return store.dispatch(orderActions.refundOrder(1)).then(() => {
        const actions = store.getActions()
    ...
    it('creates PAY_ORDER_SUCCESS when paying order has been done', () => {
      ...
      const expectedActions = [
        ...
        { type: typesNotification.ADD_NOTIFICATION, text: 'actions.order.create.payment.send.success', open: true },
    ...
    it('creates PAY_ORDER_ERROR when paying order fails', () => {
      ...
      const expectedActions = [
        { type: types.PAY_ORDER_REQUESTED, completed: false },
        { type: typesNotification.ADD_NOTIFICATION, text: 'actions.order.create.payment.send.error', open: true },
        { type: types.PAY_ORDER_ERROR, completed: true, error: new Error('Request failed with status code 500') }
      ]
      const store = mockStore({ intl: { messages: {} } })
      return store.dispatch(orderActions.payOrder({ id: 1 })).then(() => {
        const actions = store.getActions()
    ...
    it('creates CANCEL_ORDER_SUCCESS when canceling order has been done', () => {
      ...
      const expectedActions = [
        ...
        { type: typesNotification.ADD_NOTIFICATION, text: 'actions.order.cancel.success', open: true },
    ...
    it('creates CANCEL_ORDER_ERROR when canceling order fails', () => {
      ...
      const expectedActions = [
        { type: types.CANCEL_ORDER_REQUESTED, completed: false },
        { type: typesNotification.ADD_NOTIFICATION, text: 'actions.order.cancel.payment.error', open: true },
        { type: types.CANCEL_ORDER_ERROR, completed: true, error: new Error('Request failed with status code 500') }
      ]
      const store = mockStore({ intl: { messages: {} } })
      return store.dispatch(orderActions.cancelOrder(1)).then(() => {
        const actions = store.getActions()
