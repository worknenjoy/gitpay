...
describe('paymentRequestPayment actions', () => {
	describe('action creators', () => {
		it('should create paymentRequestPayment action objects', () => {
			...
			expect(paymentRequestPaymentActions.listPaymentRequestPaymentSucceeded(successPayload)).to.eql({
	...
	describe('async actions', () => {
		...
		it('creates LIST_PAYMENT_REQUEST_PAYMENT_SUCCEEDED when listing payments succeeds', () => {
			...
			return store.dispatch<any>(paymentRequestPaymentActions.listPaymentRequestPayments()).then(() => {
		...
		it('creates LIST_PAYMENT_REQUEST_PAYMENT_FAILED when listing payments fails (HTTP 500)', () => {
			...
			return store.dispatch<any>(paymentRequestPaymentActions.listPaymentRequestPayments()).then(() => {
				...
				expect(actions[0]).to.eql({ type: types.LIST_PAYMENT_REQUEST_PAYMENT_REQUESTED, payload: undefined })
