...
const multiRoleContributorProfile = {
...
const multiRoleProviderProfile = {
  username: 'alexandremagno',
  name: 'Alexandre Magno',
  website: 'https://blog.alexandremagno.net/en',
  country: 'BR',
...
const multiRoleSamplePullRequests = [
  {
    id: 1,
...
// Toggle the two Controls below to see 0, 1, or both role variants active —
// with both on, `UserProfilePublicPage` renders the combined profile: one
// shared header (with a pill per active role) followed by an Overview /
// Contributor / Service provider switcher (only the two roles with real
// variants today — Overview previews a few items from each).
export const MultiRole: Story = {
  argTypes: {
    isContributorRole: { control: 'boolean', name: 'Contributor role' },
    isProviderRole: { control: 'boolean', name: 'Service provider role' }
  },
  args: {
    isContributorRole: true,
    isProviderRole: true,
    tasks: { completed: true, data: multiRoleSampleBounties },
    pullRequests: { completed: true, data: multiRoleSamplePullRequests },
    onPayLink: action('onPayLink'),
    onViewBounty: action('onViewBounty'),
    onBountyTabChange: action('onBountyTabChange'),
    bottomBarProps
  ...
  render: (args: any) => {
    const { isContributorRole, isProviderRole, ...rest } = args
    const profileTypes = [
      ...(isContributorRole ? ['contributor'] : []),
      ...(isProviderRole ? ['provider'] : [])
    ]
    return (
      <UserProfilePublicPage
        {...rest}
        profileTypes={profileTypes}
        contributorProfile={multiRoleContributorProfile}
        providerProfile={multiRoleProviderProfile}
      />
    )
  }
}
