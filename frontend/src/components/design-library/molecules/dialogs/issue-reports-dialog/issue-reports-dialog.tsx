  ...
  return (
    ...
    >
      ...
      <DialogContent>
        ...
        <form onChange={onChangeReason} method="POST" style={{ marginTop: '20px 0px 10px', color: 'grey' }}>
          <FormControl fullWidth>
            <RadioGroup name={'reason'} value={reason} onChange={onChangeReason}>
              <FormControlLabel
                id="task.report.reason.invalid"
                value={getLanguage().localeCompare('br') === 0 ? messageBr['task.report.reason.invalid'] : messageEn['task.report.reason.invalid']}
                label={invalid}
                control={<Radio color="primary" />}
              />
              <FormControlLabel
                id="task.report.reason.fourOFour"
                value={getLanguage().localeCompare('br') === 0 ? messageBr['task.report.reason.fourOFour'] : messageEn['task.report.reason.fourOFour']}
                label={fourOFour}
                control={<Radio color="primary" />}
              />
              <FormControlLabel
                id="task.report.reason.testOrAccident"
                value={getLanguage().localeCompare('br') === 0 ? messageBr['task.report.reason.testOrAccident'] : messageEn['task.report.reason.testOrAccident']}
              ...
              <TextField
                ...
                label={getLanguage().localeCompare('br') === 0 ? messageBr['task.report.reason.custom'] : messageEn['task.report.reason.custom']}
      ...
      <DialogActions style={{ marginBottom: 10 }}>
        ...
        <Button disabled={reason.length === 0} onClick={sendReport} variant="contained" color="primary">
