...
const PayoutRequestForm = forwardRef<PayoutRequestFormHandle, PayoutRequestFormProps>(({ onSubmit, completed = true, balance, currency }, ref) => {
  ...
  const handleAddBalance = () => {
    if (amountInputRef.current) {
      ...
      if (inputElement) {
        ...
        // Trigger an 'input' event to notify the field that the value has changed
        const event = new Event('input', { bubbles: true });
        inputElement.dispatchEvent(event);

        // Focus on the input element to make the value visible and remove the placeholder
        inputElement.focus();
      }
    }
  };
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} md={12}>
          <Checkboxes
            checkboxes={[
              {
                label: <FormattedMessage id="PayoutRequest.form.confirm" defaultMessage="Convert the funds from this amount, and transfer the funds from your current balance to your registered bank account. The amount will be included in the next payout." />,
                name: 'custom_amount',
                value: true,
