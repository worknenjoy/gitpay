...
const RootPaper = styled('div')(({ theme }) => ({ width: '100%', marginTop: theme.spacing(3) }))
...
class CustomPaginationActionsTable extends React.Component {
  ...
  render() {
    const { transfers, tableHead } = this.props
    const { rowsPerPage, page } = this.state
    const emptyRows = transfers?.data?.length
      ? rowsPerPage - Math.min(rowsPerPage, transfers.data.length - page * rowsPerPage)
      : 0
    ...
    const TableRowPlaceholder = [0, 1, 2, 3, 4].map((_, rIdx) => (
