...
class TaskClaim extends Component {
  ...
  onChangeMessage = event => this.setState({ [event.target.name]: event.target.value, charactersCount: event.target.value.length })
          ...
          <DialogContent>
            ...
            <form onChange={ this.onChangeMessage } type='POST'>
              <FormControl fullWidth style={ { maxWidth: 300 } }>
                ...
                <small style={ { fontFamily: 'Roboto', color: '#a9a9a9', marginTop: '10px', textAlign: 'right' } }>{ charactersCount + '/50' }</small>
              </FormControl>
            </form>
          </DialogContent>
          <DialogActions>
