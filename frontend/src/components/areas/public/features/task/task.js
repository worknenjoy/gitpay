...
class Task extends Component {
  ...
  handlePaymentForm = (e) => {
    ...
    this.state.paymentForm
      ...
      : this.setState({ paymentForm: true, deadlineForm: false });
  };
  ...
  handleDeadlineForm = (e) => {
    ...
    this.state.deadlineForm
      ...
      : this.setState({ deadlineForm: true, paymentForm: false });
  };
  ...
  renderIssueAuthorLink = () => {
    if (this.props.task.data.metadata && this.props.task.data.metadata.issue.user.html_url) {
      return (
        <Link href={`${this.props.task.data.metadata.issue.user.html_url}`} target="_blank">
          <FormattedMessage
      ...
      return {
        ...
        description: (
          <div>
            ...
            />
            <Button
              onClick={this.handleFirstTaskBounties}
              color="primary"
              variant="contained"
      ...
      return {
        ...
        description: (
          <div>
            ...
            />
            <Button
              onClick={this.handleFirstTaskNotifications}
              color="primary"
              variant="contained"
              style={{ display: 'block', margin: '20px auto' }}
            >
  ...
  handleFundingInputMessageChange = (event) => {
    this.setState({
      fundingInvite: { ...this.state.fundingInvite, comment: event.target.value },
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'space-evenly' }}>
        <Button
          ...
          variant="contained"
          disabled={
            this.props.task.data.paid || this.props.task.data.transfer_id || this.props.task.data.Transfer
          }
          fullWidth
          style={{ marginRight: 5 }}
        >
  ...
  taskOwner = () => {
    ...
    const creator =
      this.props.logged && task.data.User && this.props.user.id === task.data.User.id;
    const owner = task.data.members && task.data.members.length
      ? task.data.members.filter((m) => m.User.id === this.props.user.id).length > 0
      : false;
  ...
  render() {
    ...
    return (
      <div>
        <Dialog open={this.state.isFirstTask} maxWidth="xs" aria-labelledby="form-dialog-title">
          <DialogContent>
                  ...
                  <div>
                    <Chip
                      ...
                      avatar={
                        <Avatar
                          className={
                            task.data.status === 'closed'
                              ? classes.avatarStatusClosed
                              : classes.avatarStatusSuccess
                          }
                      ...
                      className={
                        task.data.status === 'closed'
                          ? classes.chipStatusClosed
                          : classes.chipStatusSuccess
                      }
                    />
                  </div>
                </div>
                        ...
                        {task.data.deadline ? (
                          <div>
                            ...
                            {deadline && parseInt(deadline) > 0 ? (
                              ...
                              <Chip
                                size="small"
                                label={<FormattedMessage id="task.dealine.past" defaultMessage="Overdue" />}
                              />
                            )}
                          </div>
                        ) : (
              ...
              <OfferDrawer
                ...
                introMessage={
                  <FormattedMessage
                    ...
                    defaultMessage="You can invite a investor, sponsor, or the project owner to fund this issue and let them know your suggestions"
                  >
