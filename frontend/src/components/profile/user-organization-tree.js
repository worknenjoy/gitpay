...
export default function UserOrganizationTree({ createOrganizations, updateOrganization, organizations, user, history }) {
  ...
  return (
    <div>
      ...
      >
        { user && user.Organizations.map((o, i) => (
          <StyledTreeItem key={ i } nodeId={ i } label={
            <React.Fragment>
              <span>{ o.name }</span>
              { !o.provider ? (
                <OrganizationUpdate
                ...
                o.provider === 'github' ? (
                  <span style={ { display: 'inline-block' } }>
                    <Avatar style={ { marginLeft: 10, width: 16, height: 16, backgroundColor: 'black' } }>
                      <img width={ 12 } src={ logoGithub } alt='GitHub' />
                    </Avatar>
                  </span>
                ) : (
                  <span style={ { display: 'inline-block' } }>
                    <Avatar style={ { marginLeft: 10, width: 16, height: 16, backgroundColor: 'black' } }>
                      <img width={ 12 } src={ logoBitbucket } alt='Bitbucket' />
                    </Avatar>
                  </span>
                )
              ) }
            ...
            { o && o.Projects.map((p, j) => (
              <StyledTreeItem key={ `${i}-${j}` } nodeId={ `${i}-${j}` } label={
                <React.Fragment>
                  <span>{ p.name }</span>
                </React.Fragment>
              }>
