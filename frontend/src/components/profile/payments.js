  ...
  const displayOrders = orders => {
    ...
    return orders.map((item, i) => (
      <React.Fragment key={ item.id }>
        <tr>
          <td>{ item.paid ? intl.formatMessage(messages.labelYes) : intl.formatMessage(messages.labelNo) }</td>
          <td>
            <div style={ { display: 'inline-block' } }>
              <span style={ { display: 'inline-block', width: '100%' } }>{ statuses[item.status] }</span>
            </div>
          </td>
          <td>{ issueRow(item.Task) }</td>
          <td>${ item.amount }</td>
          <td><PaymentTypeIcon type={ item.provider } /></td>
          <td><Typography variant='caption'>{ MomentComponent(item.createdAt).fromNow() }</Typography></td>
          <td>
            <div style={ { display: 'inline-block' } }>
