        ...
        <TableRow key={ index }>
          { [0, 1, 2, 3, 4, 5].map((subIndex) => (
            <TableCell key={ subIndex }>
              <div style={ { width: 80 } }>
                <ReactPlaceholder showLoadingAnimation type='text' rows={ 1 } ready={ payouts.completed } />
              </div>
            </TableCell>
          )) }
        </TableRow>
      ...
      <Paper className={ classes.root }>
        <div className={ classes.tableWrapper }>
          <Table className={ classes.table }>
            ...
            <TableBody>
              <ReactPlaceholder style={ { marginBottom: 20, padding: 20 } } showLoadingAnimation customPlaceholder={ TableRowPlaceholder } rows={ 10 } ready={ payouts.completed }>
                { payouts.data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((n, index) => (
                  <TableRow key={ n.id }>
                    { n.map((p, subIndex) => (
