...
export const publicPaymentRequests = (state = { data: [], completed: true, error: {} }, action) => {
  switch (action.type) {
    case LIST_PUBLIC_PAYMENT_REQUESTS_REQUESTED:
      return { ...state, completed: action.completed }
    case LIST_PUBLIC_PAYMENT_REQUESTS_SUCCESS:
      return { ...state, completed: action.completed, data: action.paymentRequests }
    case LIST_PUBLIC_PAYMENT_REQUESTS_ERROR:
      return { ...state, completed: action.completed, error: action.error }
    default:
      return state
  }
