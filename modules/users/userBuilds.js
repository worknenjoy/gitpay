...
module.exports = Promise.method(function userBuilds (userParameters) {
  ...
  userParameters.activation_token = models.User.generateHash(userParameters.email)
  ...
  return models.User.build(
    ...
    .then(data => {
      if (data && data.dataValues && data.dataValues.id) {
        ...
        Sendmail.success(data.dataValues, 'Activate your account', `<p>Hi ${name || 'Gitpay user'},</p><p>Click <a href="${process.env.API_HOST}/auth/activate/?userId=${id}&token=${activation_token}">here</a> to activate your account.</p>`)
      }
      return data
    }).catch(error => {
