...
const OrderCron = {
    ...
    if (orders.length) {
      ...
      await Promise.all(orders.map(async order => {
        ...
        if (orderValues.source_id) {
          console.log(`📝 [OrderCron][verify] Checking order ID: ${orderValues.id} (source_id: ${orderValues.source_id})`);
          const orderWithDetails = await OrderDetails({ id: orderValues.id });
          if (!orderWithDetails) {
            console.log(`⚠️ [OrderCron][verify] No details found for order ID: ${orderValues.id}. Attempting to cancel...`);
            const orderCanceled = await OrderCancel({ id: orderValues.id });
            if (orderCanceled) {
              console.log(`✅ [OrderCron][verify] Order ID: ${orderValues.id} canceled successfully.`);
              invalids.push(order);
            } else {
      ...
      console.log(`🎯 [OrderCron][verify] Verification complete. Invalid orders found: ${invalids.length}`);
      return invalids;
    }
  ...
  checkExpiredPaypalOrders: async () => {
    ...
    console.log(`📦 [OrderCron][checkExpiredPaypalOrders] Found ${orders.length} PayPal orders to check.`);
    if (orders[0]) {
      try {
        ...
        await Promise.all(orders.map(async o => {
          ...
          if (order && order.status === 'succeeded' && order.paid && order.source_id) {
            console.log(`🔍 [OrderCron][checkExpiredPaypalOrders] Checking PayPal order status for order ID: ${order.id}, source_id: ${order.source_id}`);
            const orderDetails = () => requestPromise({
              ...
              console.log(`⚠️ [OrderCron][checkExpiredPaypalOrders] Error fetching order details for order ID: ${order.id}:`, e.error);
              return JSON.parse(e.error);
            });
            const orderDetailsResult = await orderDetails();
            const authorizationDetails = orderDetailsResult?.['purchase_units']?.length && orderDetailsResult['purchase_units']?.[0]?.['payments']?.[0]?.['authorizations']?.[0]
            ...
            console.log(`⏳ [OrderCron][checkExpiredPaypalOrders] Checking if authorization for order ID: ${order.id} with STATUS ${authorizationDetails?.["status"]} has expired...`);
            if (orderDetailsResult["name"] === 'RESOURCE_NOT_FOUND' || authorizationDetails["status"] === 'VOIDED' || authorizationDetails["status"] === 'CANCELED') {
              ...
              await models.Order.update({ status: 'expired', paid: false }, { where: { id: o.dataValues.id } }).then(orderUpdated => {
                if (orderUpdated[0] === 1) {
                  console.log(`✅ [OrderCron][checkExpiredPaypalOrders] Order ID: ${order.id} marked as expired. Sending notification email...`);
                  orderMail.expiredOrders(order);
                } else {
                  console.log(`❌ [OrderCron][checkExpiredPaypalOrders] Failed to update order ID: ${order.id} as expired.`);
                }
              });
            } else {
              console.log(`👍 [OrderCron][checkExpiredPaypalOrders] Order ID: ${order.id} is still valid on PayPal.`);
            }
          }
        }));
      } catch (e) {
        console.log('❗ [OrderCron][checkExpiredPaypalOrders] Error during PayPal expired order check:', e);
      }
    } else {
