...
describe('Payment Request Balance', () => {
  ...
  it('should list of Payment Request balance and related transactions for the current user', async () => {
    ...
    const paymentRequestBalanceTransaction = await currentModels.PaymentRequestBalanceTransaction.create({
    ...
    expect(res.body[0].PaymentRequestBalanceTransactions[0].id).to.equal(paymentRequestBalanceTransaction.id);
