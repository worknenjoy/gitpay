...
export const getIssue = async (issueUrl: string) => {
  ...
  const issueUrlFormatted = `https://api.github.com/repos/${owner}/${repo}/issues/${issueNumber}`
  const issue = await GithubConnect({ uri: issueUrlFormatted })
  return issue
}
