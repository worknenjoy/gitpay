...
async function fetchAllStripeData(listFn:any, label: string, params = {}) {
  ...
  while (hasMore) {
    page++;
    console.log(`[Stripe] ${label}: requesting page ${page}${startingAfter ? ` (starting_after=${startingAfter})` : ""}`);
    ...
    console.log(`[Stripe] ${label}: received ${response.data.length} items, total=${results.length}, has_more=${response.has_more}`);
...
async function getStripeData() {
  ...
  const transfers = await fetchAllStripeData((params:any) => stripe.transfers.list(params), "transfers");
  const payouts = await fetchAllStripeData((params:any) => stripe.payouts.list(params), "payouts");
  const invoices = await fetchAllStripeData((params:any) => stripe.invoices.list(params), "invoices");
  ...
  for (const p of payments) {
    ...
    if (p.balance_transaction) {
      try {
        ...
        console.warn(`[Stripe] Failed to retrieve balance transaction ${p.balance_transaction} for charge ${p.id}: ${err?.message || err}`);
      }
    }
    if (idx % 50 === 0 || idx === totalCount) {
      console.log(`[Stripe] Processed ${idx}/${totalCount} charges... so far: payments=$${totalPayments.toFixed(2)}, refunds=$${totalRefunds.toFixed(2)}, fees=$${totalStripeFees.toFixed(2)}`);
    }
  }
  ...
  console.log(`[Stripe] Aggregates -> payments=$${totalPayments.toFixed(2)}, refunds=$${totalRefunds.toFixed(2)}, fees=$${totalStripeFees.toFixed(2)}, transfers=$${totalTransfers.toFixed(2)}, payouts=$${totalPayouts.toFixed(2)}, subscriptions=$${subscriptionRevenue.toFixed(2)}`);
