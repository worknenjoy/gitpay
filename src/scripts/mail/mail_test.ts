...
function printHelp() {
	...
	console.log('  tsx src/scripts/mail/mail_test.ts PaymentRequestMail.transferInitiatedForPaymentRequest --out tmp/mail-previews')
}
...
async function run(): Promise<PreviewResult> {
	...
	const defaultTemplatePath = path.join(repoRoot, 'src', 'modules', 'mail', 'templates', 'default.js')
	// eslint-disable-next-line @typescript-eslint/no-var-requires
	...
	if (!rawContent) {
		throw new Error('Could not extract email HTML content from the result. Ensure NODE_ENV=test and that the mail method uses the request() helper.')
	}
