...
function dim(s: string) { return color(s, c.gray); }
...
function getCliParams(): CliParams {
  ...
  const paymentRequestId =
    readArg(['--paymentRequestId', '-r', '-pr']) ??
    process.env.PAYMENT_REQUEST_ID ??
    null;
...
async function main() {
  ...
  try {
    ...
    kv('Customer', intent.customer ?? intent.customer_details ?? intent.payment_method?.billing_details?.email ?? '-');
