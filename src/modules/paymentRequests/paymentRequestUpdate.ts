...
module.exports = async function paymentRequestUpdate(paymentRequestParams: any) {
  ...
  const stripePaymentLinkActiveUpdate = await stripe.paymentLinks.update(paymentRequestUpdate.payment_link_id, {
  ...
  const stripeProductUpdate = await stripe.products.update(
    productItemId,
    {
