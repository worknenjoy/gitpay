...
if (constants.canSendEmail) {
  ...
  TaskMail.notifyPayment = async (user, data) => {
    ...
    targetUsers.map((u, i) => {
      ...
      i18n.setLocale(language)
      mailList.push(u.email)
      subjectData.push(i18n.__('mail.issue.payment.subject', { value: data.task.value, title: data.task.title }))
      templateData.push({ ...data,
        content: {
          ...
          docs: i18n.__('mail.issue.docs.title'),
          reason: i18n.__('mail.issue.reason'),
          subject: i18n.__('mail.issue.payment.subject', { value: data.task.value, title: data.task.title })
        } })
    })
    return withTemplate(
      mailList,
