...
module.exports = Promise.method(async function userBuilds (userParameters) {
  ...
  try {
    ...
    if (dataValues && dataValues.id) {
      ...
      if(!userParameters.provider) {
        Sendmail.success(dataValues, 'Activate your account', `<p>Hi ${name || 'Gitpay user'},</p><p>Click <a href="${process.env.FRONTEND_HOST}/#/activate/user/${id}/token/${activation_token}">here</a> to activate your account.</p>`)
      }
