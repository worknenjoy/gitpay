  ...
  try {
    const data = await currentModels.Task.findOne({
      ...
      include: [
        ...
        {
          model: currentModels.History
        },
        {
          model: currentModels.Label
        },
        {
          model: currentModels.Transfer
        }
      ]
    })
        ...
        try {
          ...
          if (data.Labels.length !== issueDataJsonGithub.labels.length) {
            ...
            try {
              ...
              githubLabels.forEach(async (l: any) => {
                if (!taskLabels.includes(l['name'])) {
                  const label = await currentModels.Label.findOrCreate({
                    where: {
                      name: l['name']
                    },
